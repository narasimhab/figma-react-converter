...
function wrapHtml(html) {
  ...
  if (/^<body/i.test(trimmed)) {
    return "<!doctype html><html><head><meta charset='utf-8'></head>" + trimmed + "</html>";
  }
  return (
...
function walk(el, containerRect, win, atoms) {
  ...
  const box = extractBoxStyle(el, cs, rect, containerRect, tag);
...
function extractBoxStyle(el, cs, rect, containerRect, tag) {
  ...
  if (tag === "i" || tag === "svg") {
    return {
      ...
      x, y, w, h,
      color: cs.color,
      bg, fontFamily: cs.fontFamily, fontSize: parseFloat(cs.fontSize) || 14,
    };
  }
